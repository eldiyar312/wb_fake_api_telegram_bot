...
import { handleCallbakQueryMessage, handleMessage } from '../TGMessage/handle'
...
const updateCommands = ['/edit_product']
  ...
  try {
    ...
    if (body.callback_query) {
      if (deleteCommands.indexOf(body.callback_query.data) !== -1) {
        chatLastCommad[body.callback_query.message.chat.id] = body.callback_query.message.text
      ...
      chatLastCommad[body.callback_query.message.chat.id] && handleCallbakQueryMessage(body.callback_query, chatLastCommad[body.callback_query.message.chat.id])
    }
  } catch (error) {
